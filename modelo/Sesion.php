	...
	{   
		...
		{	
			...
			if(isset($_REQUEST["email"]))
			{
				...
				{
					setcookie("losboletos", md5($user[0]["id"]), time()+ (60 * 60 * 24 * 7), "/", $_REQUEST["server"]);
		...
		{	
			$comando_sql	="
				...
				FROM user u
				WHERE 
					md5(id)='{$_COOKIE["losboletos"]}'
			";
			
			$user	=$this->__EXECUTE($comando_sql);								
			if(isset($user[0]) and isset($user[0]["md5_id"]) and $user[0]["md5_id"]==$_COOKIE["losboletos"])
			{				
